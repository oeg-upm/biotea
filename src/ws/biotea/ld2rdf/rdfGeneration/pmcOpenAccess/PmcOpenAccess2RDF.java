...
//        SAXSource myPaper = new SAXSource(reader, new InputSource(new FileInputStream(paper))); 	
		try {
		...
		if (pmcID == null) {
			throw new PMCIdException("No PMC id was found, file cannot be processed");
		}
	...
	public File paper2rdf(String outputDir, File paper, boolean sections) throws JAXBException, FileNotFoundException, UnsupportedEncodingException {	
		...
		try {
			...
			if (ArticleType.valueOf(type).getBiboType() != null) {
				Statement stm = model.createStatement(document, Thing.RDF_TYPE, ArticleType.valueOf(type).getBiboTypeURI());
			    model.addStatement(stm); //rdf:type
			}
			...
			if (fatalError) {
				outputFile = serializeAndCloseModel(model, outputDir + "/PMC" + pmcID + ".rdf");		
		...
		if (sections) {
			...
			try {
				...
				if (fatalError || fatalErrorSections) {
					//close and write model
					outputFileSections = serializeAndCloseModel(modelSections, outputDir + "/PMC" + pmcID + "_sections.rdf");		
	...
	private Model createAndOpenModel() {
		...
		myModel.open();					
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.OWL.getNS(), ws.biotea.ld2rdf.util.Prefix.OWL.getURL());
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.RDF.getNS(), ws.biotea.ld2rdf.util.Prefix.RDF.getURL());
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.RDFS.getNS(), ws.biotea.ld2rdf.util.Prefix.RDFS.getURL());
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.BIBO.getNS(), ws.biotea.ld2rdf.util.Prefix.BIBO.getURL());
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.DOCO.getNS(), ws.biotea.ld2rdf.util.Prefix.DOCO.getURL());
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.FOAF.getNS(), ws.biotea.ld2rdf.util.Prefix.FOAF.getURL());
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.DCTERMS.getNS(), ws.biotea.ld2rdf.util.Prefix.DCTERMS.getURL());
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.FOAF.getNS(), ws.biotea.ld2rdf.util.Prefix.FOAF.getURL());		
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.XSP.getNS(), ws.biotea.ld2rdf.util.Prefix.XSP.getURL());		
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.CNT.getNS(), ws.biotea.ld2rdf.util.Prefix.CNT.getURL());
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.PROV.getNS(), ws.biotea.ld2rdf.util.Prefix.PROV.getURL());
		myModel.setNamespace(ws.biotea.ld2rdf.util.Prefix.VOID.getNS(), ws.biotea.ld2rdf.util.Prefix.VOID.getURL());
		return (myModel);
	}
	/**
...
//		pw.write(serialization);
	...
	private void processBasic(Model model, Document document, String paper) {
		...
		if (pubmedID != null) {
		    ...
		    if (Config.withBio()) {
		    	document.addSameAs(model, Config.IDENTIFIERS_ORG_PUBMED + pubmedID);
			    document.addSameAs(model, Config.BIO2RDF_PUBMED + pubmedID);
		    }		    
		    if (!global.getUriStyle().equals(Config.bio2rdf)) {
...
//		    document.addSeeAlso(resDOI);
		...
		try {
			...
			if (publisherPMCId != null) {//we create the publisher with the id
			    ...
			    Statement stm = model.createStatement(publisher, Document.DCTERMS_IDENTIFIER, id);
			    model.addStatement(stm); //id
	...
	private void processAuthors(Model model, Document document, String paper) {
		try {
		    ...
		    //list in rdf: Config.BIOTEA_URL + "/{0}/pmc_resource/" + pmcID + "/";
	...
	private Document processReferenceCreateArticle(Model model, Document document, String url, String pubmed, String doiReference, Ref ref, ReferenceType type) {		
		...
		if (pubmed != null) {
			...
			docReference.addSameAs(model, Config.IDENTIFIERS_ORG_PUBMED + pubmed); //sameAs for entities
			docReference.addSameAs(model, Config.BIO2RDF_PUBMED + pubmed); //sameAs for entities
    	...
    	try {
    		if (type == ReferenceType.JOURNAL_ARTICLE) {
        		...
        		if (sourceId != null) {
        		    ...
        		    //TODO Publisher
//        		    PlainLiteral publisher = model.createPlainLiteral(sourceId);	
//    			    journal.addbiboPublisher(publisher);
//    			    docReference.addbiboPublisher(publisher);
        		    //Issue
        		    if (journal != null) {
    		    ...
    		    /*if (publisherName != null) {//TODO: it should be a link (foaf:Agent)
		    ...
		    /*if (publisherName != null) {//TODO should be a link
		    	Node publisherNode = new PlainLiteralImpl(publisherName);
	...
	private String processElement(Model model, Document document, Object obj, SectionE secDoco, String secDocoURI) {
		...
		if (obj instanceof Sc) {
			...
			return str;			
		} else if ((obj instanceof Fig) || (obj instanceof TableWrap)) { //TODO why nothing?
			return "";
			...
			try {
				...
				Statement stm = model.createStatement(image, Thing.RDF_TYPE, Figure.RDFS_CLASS);
	...
	private String processAbstractAsSection(Abstract ab, Model model, Document document, SectionE parent) {
		...
		/*Statement stmContent = model.createStatement(paragraph, Thing.RDF_TYPE, new URIImpl(Publication2RDF.PUB_CONTENT_CLASS, false));
		model.addStatement(stmContent);*/ //Do not include Content as type as it is implicit from ContentAsText
		Statement stmContentAsText = model.createStatement(paragraph, Thing.RDF_TYPE, new URIImpl(Publication2RDF.PUB_CONTENT_AS_TEXT_CLASS, false));
		model.addStatement(stmContentAsText);
		secDoco.addParagraph(model, paragraph);		
		PlainLiteral textNode = model.createPlainLiteral(text);	 
	    Statement stm = model.createStatement(paragraph, new URIImpl(PmcOpenAccessHelper.TEXT_PROPERTY, false), textNode);
	    model.addStatement(stm); //text
		return text;
	}
	/**
	 * Processes a paragraph within the abstract.
		...
		for (String[] refs:references) {			
			for (String ref:refs) { 						
				if ( !( ref.startsWith("Fig") || ref.startsWith("Tab") || ref.startsWith("fig") || ref.startsWith("tab") ) ) {
					...
					Statement stm = model.createStatement(table, Thing.RDF_TYPE, DocoTable.RDFS_CLASS);
				    model.addStatement(stm); //rdf:type Document
	...
	private void processParagraph(Model model, Document document, String titleInURL, int countPara, SectionE secDoco, P para) {
		...
		model.addStatement(stmContentAsText);
		PlainLiteral textNode = model.createPlainLiteral(text);
		Statement stmChars = model.createStatement(paragraph, new URIImpl(Publication2RDF.PUB_CONTENT_CHARS_PROP, false), textNode);
	    model.addStatement(stmChars); //text
	    PlainLiteral encodingNode = model.createPlainLiteral(Publication2RDF.UTF_ENCODING);
	    Statement stmEncoding = model.createStatement(paragraph, new URIImpl(Publication2RDF.PUB_CONTENT_CHARS_ENCODING_PROP, false), encodingNode);
	    model.addStatement(stmEncoding); 
	}
	/**
