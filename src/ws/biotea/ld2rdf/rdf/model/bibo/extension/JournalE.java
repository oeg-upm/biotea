...
public class JournalE extends Journal {
	...
	public void addIssue(Model model, Issue issue) {	    
	    Statement stm = model.createStatement(this, DCTERMS_HAS_PART, issue);
	    model.addStatement(stm); //journal hasPart issue
	    Statement inv = model.createStatement(issue, DCTERMS_IS_PART_OF, this);
	...
	public void addDocument(Model model, ws.biotea.ld2rdf.rdf.model.bibo.Document doc) {	    
	    Statement stm = model.createStatement(this, DCTERMS_HAS_PART, doc);
	    model.addStatement(stm); //journal hasPart issue
	    Statement inv = model.createStatement(doc, DCTERMS_IS_PART_OF, this);
	    model.addStatement(inv); //section isPartOf document
	}
	...
	public void addISSN(Model model, String issn) {
	    ...
	    Statement stm = model.createStatement(this, BIBO_ISSN, doiAsLiteral);
